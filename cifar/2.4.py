...
# 隐私预算为7时，our 谷歌 固定精度对比res34+cifar100
# 加载数据
with open('plant_data/2.2-our-dp7-43.56.txt', 'r') as f1:
    data1 = f1.readlines()  # 加载第一个文件的数据

with open('plant_data/2.2-gg-dp7-39.06.txt', 'r') as f2:
    data2 = f2.readlines()  # 加载第二个文件的数据

with open('plant_data/2.2-gd5-dp7-42.29.txt', 'r') as f3:
    data3 = f3.readlines()  # 加载第三个文件的数据

with open('plant_data/2.2-gd0.5-dp7-43.08.txt', 'r') as f4:
    data4 = f4.readlines()  # 加载第四个文件的数据

with open('plant_data/2.2-zxj-dp7-41.41.txt', 'r') as f5:
...
plt.plot(rounds, acc3[::5], color='#73BCD5', marker='s', label='Fixed (C=5)')
